  ...
  async fetch(request, env2, ctx) {
    ...
    try {
      if (path === "/api/gallery/images" && request.method === "GET") {
        const stmt = env2.DB.prepare("SELECT artwork_id, image_data, uploaded_at FROM gallery_images WHERE user_id = ?");
        ...
        const stmt = env2.DB.prepare("DELETE FROM gallery_images WHERE user_id = ? AND artwork_id = ?");
